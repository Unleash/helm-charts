...
- Helm charts live in `charts/`: `unleash`, `unleash-proxy`, and `unleash-edge`. Each chart contains `Chart.yaml`, `values.yaml`, `templates/`, `templates/tests/`, `tests/`, and `examples/` for sample overrides.
...
- Run template unit tests: `helm unittest charts/unleash` (requires the helm-unittest plugin).
...
- Add or adjust `tests/` helm-unittest suites for template branching or rendered manifest behavior; add or adjust `templates/tests/` only for Helm test hooks that run inside a cluster.
- Run `helm unittest` and `ct lint` before opening a PR; use `helm template` plus `.github/kubeconform.sh` for schema confidence against supported Kubernetes versions.
